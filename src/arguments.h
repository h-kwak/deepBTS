    ...
    bool get(int argc, char *argv[])
    {
        ...
        for(int i=0;i<n;i++)
        {
            if(is_optional[i]==false&&is_passed[i]==false)
            {
				...
				std::cerr<<"\t"<<argv0.substr(argv0.find_last_of('/')+1);
				bool anyoptional=false;
				for(int j=0;j<n;++j) if(is_optional[j]) anyoptional=true;
				if(anyoptional) std::cerr<<" [OPTIONS]";
				for(int j=0;j<n;++j) if(!is_optional[j])
				{
					std::cerr<<" "<<id_string[j]<<" "<<description[j];
				}
				std::cerr<<std::endl<<std::endl;

				if(anyoptional) std::cerr<<"Option\tDescription"<<std::endl;
                for(int j=0;j<n;j++) if(is_optional[j])
					std::cerr<<id_string[j]<<"\t"<<description[j]<<std::endl;
                return false;
            }
        }
        return true;
